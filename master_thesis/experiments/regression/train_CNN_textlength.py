...
START = 0 # None
LR = 1e-4
FRACTION = 1 # #1 #TODO: change back to 1

TARGET = 'avgTimeOnPage'

# building identifier from hyperparameters (for Tensorboard and saving model)
identifier = f"CNN_textlength_FIXLEN{FIXED_LEN}_MINLEN{MIN_LEN}_START{START}_EP{EPOCHS}_BS{BATCH_SIZE}_LR{LR}_{TARGET}_NOZ"
...
model = models.CNN_textlength(num_outputs = 1,
                   embs_dim = EMBS_DIM,
                   filter_sizes=[3, 4, 5],
                   num_filters=[30,30,30] #[100,100,100]
                   )
...
def evaluate_model(model):
    ...
    with torch.no_grad():
        for nr, d in enumerate(dl_dev):
            ...
            outputs = model(input_matrix= input_matrix, textlength = textlength)
            # print(outputs[:10])
